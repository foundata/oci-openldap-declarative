...
On the administration host, use the variables from the
...
On the LDAP host, set `revision` to the staged revision. Run deployments one at
a time. The subshell stops at the first error; a rejected preflight leaves the
...
## Verify and connect an application
...
Increment the service's YAML revision and regenerate before soft expiry.
Transfer to a new staging directory, then repeat preflight, activation and
verification. Replaying an existing artifact does not renew it.
...
For signing-key rotation, configure the runtime and preflight with a directory
containing old and new `*.pub` keys using `LDAP_SNAPSHOT_PUBLIC_KEY_DIR`; do not
also set `LDAP_SNAPSHOT_PUBLIC_KEY_FILE`. Restart with that trust set before
signing the next revision with the new key.
...
After confirming the new revision everywhere, remove the old key from the
runtime trust directory. Empty key directories and symlinks are rejected.
